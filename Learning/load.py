...
class MultiTargetLightDarkDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample = self.collect_target(traj, i)
    ...
    def collect_target(self, traj, i):
        # truncate & fit interface of sample to model
        o, a, r, next_a, next_s, next_r, timestep, mask = [], [], [], [], [], [], [], []
        o = traj['observation'][:i].reshape(1, -1, 2)
        a = traj['action'][:i].reshape(1, -1, 2)
        r = traj['reward'][:i].reshape(1, -1, 1)
        next_a.append(np.round(traj['action'][i], 4).reshape(1, -1, 2))
        next_r = traj['reward'][i].reshape(1, -1, 1)
        next_s = traj['next_state'][1:i+1].reshape(1, -1, 2)
        timestep = np.arange(0, i).reshape(1, -1)
        timestep[-1][timestep[-1] >= 31] = 31 - 1  # padding cutoff
        ...
        if i == 1:
            ...
            for idx in range(len(dataset)):
                if np.array_equal(np.round(dataset[idx]['observation'][0] ,4), o0):
            ...
            for idx in range(len(dataset)):
                if np.array_equal(np.round(dataset[idx]['action'][1], 4), a1) and np.array_equal(np.round(dataset[idx]['observation'][1], 4), o1):
                    target_index.append(idx)
            ...
            for idx in range(len(dataset)):
                if np.array_equal(np.round(dataset[idx]['action'][1], 4), a1) and np.array_equal(np.round(dataset[idx]['observation'][1], 4), o1) and np.array_equal(np.round(dataset[idx]['action'][2], 4), a2) and np.array_equal(np.round(dataset[idx]['observation'][2], 4), o2):
                    target_index.append(idx)

        # Collect multi-targets
        for t in target_index:
            next_a.append(np.round(dataset[t]['action'][i], 4)).reshape(1, -1, 2)
        ...
        next_a = np.array(next_a) concate
