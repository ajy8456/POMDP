...
class Settings(Serializable):
    ...
    dataset_file: str = 'light_dark_long_mini.pickle'
...
def collect_target(config, dataset):
    ...
    for t in target_index:
        if dataset[t]['traj_len'] == 2:
            continue
        targets.append(dataset[t]['action'][2])
...
def main():
    ...
    data, targets = collect_target(config, dataset)
    ...
    pred_GPT, time_GPT = predict_action(config, model_GPT, data)
