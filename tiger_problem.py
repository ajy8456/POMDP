...
from POMCP import *
from ref.visual import visualize_pouct_search_tree
import random
import numpy as np
import sys
...
def main():
    init_true_state = random.choice([State("tiger-left"),
                                     State("tiger-right")])
    init_belief = Histogram({State("tiger-left"): 0.5,
                                      State("tiger-right"): 0.5})
    tiger_problem = TigerProblem(0.15,  # observation noise
                                 init_true_state, init_belief)

    print("** Testing POMCP **")
    tiger_problem.agent.set_belief(Particles.from_histogram(init_belief, num_particles=100), prior=True)
    pomcp = POMCP(max_depth=5, discount_factor=0.95,
